...
### Fixed
- **Minimap tracking could lock onto the wrong champion, permanently.** This is
  the root cause behind "teammates are loud wherever I am", "enemies went silent
  after I alt-tabbed", and "it's just not reliable" — all three. A real session
  log showed the tracker re-locking eleven times in 63 seconds, every time onto
  a blob scoring 0.23-0.29 that the classifier had rated 0.000, while a healthy
  lock scores 0.74. Once it settled on someone else's icon it followed them
  happily, so the 5-second hold that was the only way back to scanning never
  triggered — and their coordinates went out as ours at 10 Hz for the rest of
  the game. Locking now requires a real confidence score, and a lock that
  another icon out-scores for several seconds is abandoned.
- **The overlay was resizing its own window 40 times a second, forever.** A
  feedback loop between the window resize and the ResizeObserver that measured
  it; 3629 of 4180 lines in that log were this one message. It saturated the
  thread the minimap scan and the volume tick run on, which is why everything
  else felt flaky. Resizes now need a real layout change.
- **Alt-tabbing fed the tracker the desktop.** The screen capture had no notion
  of the League window — it grabbed a fixed rectangle whatever was in front. It
  now reports when the game is not in the foreground, and those frames are
  skipped instead of being treated as a minimap.
- **A stalled capture froze tracking permanently and silently.** One promise
  that never settled was enough; there was no timeout and nothing logged.
- Successful re-acquisition did not clear the hold timer, so enemies stayed
  needlessly muted for up to five seconds — and the stale hold then triggered
  the forced re-lock that could land on the wrong champion.
- After a dropped signalling connection, roughly half the room was lost for the
  rest of the game: peers tore down their side, we kept ours, and whoever was
  the designated initiator never re-offered.
- Touching the Scan Rate slider permanently killed the overlay's live refresh.

### Added
- **Stereo positioning.** Voices come from the direction the speaker is
  standing, using only the champion icons visible on **your own** minimap.
  Nothing is exchanged between clients — an icon you can see is a position the
  game already gave you, which is what makes this possible for enemies at all;
  sending coordinates over the network would have been a wallhack. Fog of war
  therefore works for free: someone you cannot see stays centred.
  Attribution uses scarcity (the server only sends peers within ~1350 units, so
  usually exactly one is audible), the distance the server encodes in the
  volume, and continuity across frames. When two candidates remain plausible it
  stays centred rather than guessing — a wrong direction is worse than none.
- **River reverb.** Voices from the river get a little reverb, taken from the
  speaker's surroundings. The river footprint is extracted from the game's own
  minimap artwork by `scripts/generate-river-mask.mjs` rather than estimated
  from coordinates. Speakers you cannot see stay dry.
- **Reset All** in Settings — every setting back to factory, with a two-click
  confirm. Also restores the key bindings held on the Rust side, which a plain
  storage wipe would have left silently active.
- **A real app icon.** The previous one was a placeholder of solid red, in a
  single 32x32 size that Windows scaled everywhere. Now a microphone with range
  arcs, generated by `scripts/generate-app-icon.mjs` at seven sizes.
- A **tracking badge** in the panel header. Tracking health was previously only
  visible in an element that disappears as soon as anyone joins — so mid-game,
  when it matters, there was no way to tell.

### Changed
- **Min Vol, Fade Start and Fade Curve are gone from Settings** — too fiddly to
  be useful. They keep working at sensible fixed values, and any stored value is
  ignored rather than left stuck somewhere invisible.
- New **Stereo** (0-150%) and **Reverb** controls; **Audio Boost** moved to the
  troubleshooting section, since Stereo and Reverb also depend on it.

### Notes
- Stereo needs your own position, so it depends on the tracking fixes above.
- Two enemies standing close together merge into one icon on the minimap; when
  that happens their voices stay centred.
- Client tests: 201 (was 162).
